...
def test_login_browser_is_non_root_and_keeps_cdp_and_vnc_loopback_only() -> None:
    ...
    # Instagram can retain a valid session behind account confirmation pages.
    # Keep any such page visible for a human decision instead of treating it
    # as a completed profile check.
    assert "accounts(?:\\\\/|$)" in service
