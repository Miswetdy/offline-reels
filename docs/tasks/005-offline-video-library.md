...
TASK-005 Block 6.3 prepares real-iPhone acceptance without adding a deployment stack. The client has no implicit production localhost API origin: it requires a valid public `NEXT_PUBLIC_API_BASE_URL`; the acceptance procedure uses separate HTTPS frontend/API tunnels and configures `FRONTEND_ORIGIN` to the frontend tunnel for CORS. The manifest starts the standalone PWA at `/offline`, exposes an icon and the existing worker scope `/`; local offline media remains same-origin. The `/offline` summary includes exact library bytes, approximate estimate/quota where available, and a non-invasive `navigator.storage.persisted()` diagnostic. An acceptance worksheet records the mandatory Airplane Mode, ten-video, Range, lifecycle, delete/clear, worker-update, reconciliation and no-crash thresholds. Actual iPhone acceptance is still pending.
