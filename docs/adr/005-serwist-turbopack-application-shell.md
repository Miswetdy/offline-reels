...
After a production user has visited `/offline`, the page shell can be reloaded offline and read the local IndexedDB catalog. Offline `/videos` opens its cached shell but never presents an old Backend catalog; it links to `/offline` instead. The network indicator is informational only. Development intentionally disables worker registration. Native `esbuild` must be available for production builds on Windows. The worker does not force `skipWaiting`, and `reloadOnOnline=false`. When Serwist reports a waiting worker, a compact notification lets the user explicitly request its documented message-based `SKIP_WAITING` activation. Only the following `controllerchange` reloads the page, once; neither this flow nor shell-cache cleanup touches `offline-reels-media-v1` or IndexedDB.
